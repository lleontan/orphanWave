...
const api = require('./helpers/apiFetchIEX');
...
app.get('/test/unemployment', async (req, res) => {
  const retData = await IexApi.iexGetRequest(constants.IEX.UNEMPLOYMENT_RATE_DATAPOINTS_ROUTE).then(
    (data) => {
      res.send(data);
    },
  );
...
const port = normalizePort(constants.LOCAL_PORT || '9000');
